...
date_time_check = datetime.strptime(date_check, '%d/%m/%Y')
class PlayerSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for news in response.css('article'):
            link = 'https://www.goal.com/' + news.css('a').attrib['href']
        ...
        date_time = datetime.strptime(date.strip(), '%d/%m/%Y')

        if next_page is not None:
            if date_time >= date_time_check:
